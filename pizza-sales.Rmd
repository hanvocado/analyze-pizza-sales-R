...
# Phần 3. Mục tiêu phân tích:
  Trong bối cảnh kinh doanh ngành dịch vụ ẩm thực ngày càng cạnh tranh, việc hiểu rõ hành vi tiêu dùng và dự đoán nhu cầu của khách hàng đóng vai trò quan trọng trong việc đưa ra các chiến lược kinh doanh hiệu quả. Nghiên cứu này tập trung phân tích dữ liệu bán hàng pizza của một cửa hàng trong năm 2015, từ đó khám phá các yếu tố ảnh hưởng đến nhu cầu tiêu thụ và doanh thu.
  Bằng cách áp dụng các phương pháp tiền xử lý dữ liệu, phân tích thống kê, mô hình hóa và trực quan hóa dữ liệu, nghiên cứu nhằm đạt được các mục tiêu:
  
  Phân tích và trực quan hóa nhu cầu tiêu thụ pizza theo thời gian, xác định rõ xu hướng tiêu thụ theo các giai đoạn cụ thể như mùa, tháng, tuần, ngày trong tuần và giờ trong ngày. Qua đó, nghiên cứu đánh giá sự khác biệt về doanh số dựa trên kích cỡ bánh, phân loại theo danh mục bánh, và xác định các loại pizza bán chạy nhất cũng như kém phổ biến nhất. Bằng cách xây dựng các mô hình dự đoán như hồi quy tuyến tính và Random Forest, nghiên cứu đạt được độ chính xác trên 80% trong việc dự đoán doanh số bán hàng theo từng loại sản phẩm và doanh thu của cửa hàng theo thời gian. Dựa trên kết quả phân tích hành vi tiêu dùng, nghiên cứu đề xuất các chiến lược kinh doanh như tập trung vào các thời điểm nhu cầu cao, tối ưu hóa danh mục sản phẩm, và điều chỉnh chương trình khuyến mãi phù hợp để tối đa hóa doanh thu và cải thiện lợi nhuận. Các kết quả thu được không chỉ cung cấp hiểu biết sâu sắc về hành vi khách hàng mà còn hỗ trợ xây dựng chiến lược kinh doanh hiệu quả và bền vững.
...
* Kiểm định sự chênh lệch trong doanh thu giữa các tháng của mỗi bánh có ý nghĩa thống kê không bằng Kruskal-Wallis

Kruskal-Wallis là một kiểm định phi tham số, đặc biệt được dùng khi biến không có phân phối chuẩn, phù hợp để so sánh sự biến thiên của doanh thu giữa các tháng cho từng pizza.
...
Kết quả p-value khoảng 0.4 trong kiểm định Kruskal-Wallis có thể chỉ ra rằng không có sự khác biệt đáng kể trong doanh thu giữa các tháng đối với mỗi pizza, chứng tỏ doanh thu giữa các tháng tương đối ổn định.

# Phần 7. Dự đoán doanh số theo sản phẩm:

Mục tiêu của phần này là xây dựng mô hình dự đoán doanh số của một bánh pizza với các đặc trưng của nó.

* Tổng hợp dữ liệu chứa thông tin doanh số trong mỗi tháng
...
summary(sales_data)
```
Dữ liệu gồm 1091 quan sát, mỗi quan sát thể hiện doanh thu của một bánh với kích cỡ xác định (hoặc S hoặc M hoặc L hoặc XL hoặc XXL), đơn giá tương ứng, thuộc danh mục nào và doanh số trong một tháng cụ thể là bao nhiêu. 

Biến sales là biến phụ thuộc mà ta xây dựng mô hình để dự đoán, nó dao động từ 1 đến 444, với trung bình là 115, đây sẽ là cơ sở để đánh giá mô hình.

Các biến độc lập bao gồm: pizza_name, unit_price, pizza_size và pizza_category. Như đã kiểm định ở trên doanh thu giữa các tháng của một bánh pizza tương đối ổn định, nên order_month không là biến độc lập.
...
## 7.1. Dự đoán doanh thu của sản phẩm bằng hồi quy tuyến tính
...
Thông tin về mô hình:
  ...
  + Mô hình đã giải thích được 69.73% sự biến động của doanh số dựa trên các đặc trưng đầu vào.

> Mô hình khá đáng tin cậy do RMSE thấp và phù hợp với bài toán với khả năng dự đoán ổn (69.7%) nhưng vẫn còn có thể cải thiện.
...
```
  ...
  + Mô hình đã giải thích được 90% biến thiên của sales trên tập kiểm tra.
...
```
  ...
  + Giá trị dự đoán và giá trị thực tế của doanh thu chênh lệch nhau khoảng 9 đơn vị, bằng khoảng 8% giá trị trung bình của sales.

> Tập huấn luyện và tập kiểm tra đều có R-squared cao khoảng 90%, nên có thể nói rằng mô hình không bị overfitting và đạt hiệu quả dự đoán rất tốt và đáng tin cậy. Mô hình phù hợp cho bài toán và có khả năng dự đoán chính xác cao.

## Kết luận

# Phần 8. Dự đoán doanh thu theo thời gian:

+ Trước khu đưa ra những dự đoán về doanh thu theo thời gian, chúng ta cần phân tích tổng quan về doanh thu theo thời gian. 

# 8.1. Phân tích doanh thu theo thời gian:

+ Phân tích tổng quan đã cho thấy trong năm 2015, cửa hàng có tổng doanh thu là 817.860 USD. Với giá trị đơn hàng nhỏ nhất là 9.75 USD, lớn nhất là 444.40 USD. Tổng giá trị của một đơn hàng bằng giá mỗi loại pizza*số lượng pizza đó trong một đơn hàng. Cột total_price ở dữ liệu ban đầu cho biết tổng giá của từng loại pizza trong một đơn hàng được đặt.

+ Phần dƯới đây, sẽ phân tích chi tiết hơn về doanh thu bán được của cửa hàng theo thời gian giờ trong ngày, tuần, và tháng trong năm 2015.

# 8.1.1. Phân tích doanh thu theo giờ trong ngày của một tuần:
...
# 8.1.2. Phân tích doanh thu một ngày dùng Time Series:
...
# 8.1.3. Phân tích doanh thu theo tháng:
...
# 8.2. Dự đoán doanh thu trong 6 tháng tiếp theo bằng mô hình ARIMA: 
...
lines(revenue_ts, col = "blue", lwd = 2)
```
...
3. Nhận xét về độ tin cậy: Với khoảng tin cậy khá rộng, đặc biệt ở cuối giai đoạn dự báo, điều này có thể ám chỉ rằng mô hình gặp khó khăn trong việc nắm bắt toàn bộ đặc điểm của dữ liệu, đặc biệt là những biến động bất thường trong lịch sử.
...
# 8.3. Dự đoán doanh thu trong 6 tháng tới bằng Random Forest:
