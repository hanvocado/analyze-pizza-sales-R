...
```
Biểu đồ cho thấy có mối quan hệ tương quan giữa doanh số với giá bán, kích cỡ và danh mục. Đồng thời, có sự biến động lớn trong sales nên khi tạo mô hình hồi quy tuyến tính, em sẽ sử dụng logarithmic transformation trên biến sales.
...
set.seed(123)  # Để đảm bảo kết quả có thể tái tạo
train_index <- sample(1:nrow(sales_data), 0.7 * nrow(sales_data))
...
rf_model <- randomForest(sales ~ unit_price + pizza_size + pizza_category, data = train_data)
...
```
```{r}
# Đánh giá độ quan trọng của các biến
importance(rf_model)
```
