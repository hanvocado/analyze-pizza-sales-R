...
by_order_df <- df %>% group_by(order_id) %>% 
  summarise(quantity = sum(quantity), order_date = order_date[1], total_price = sum(total_price), order_hour = order_hour[1], order_weekday = order_weekday[1], order_month = order_month[1])
...
```{r}
df %>% select(order_id) %>% unique() %>% count()
```
```{r}
df %>% summarise(no_pizzas = sum(quantity), total_revenue = sum(total_price))
```
Trong năm 2015, cửa hàng có 21350 đơn đặt hàng, bán ra tổng 49574 chiến bánh pizza với tổng doanh thu là 817860 USD. 

```{r}
df %>% group_by(order_id) %>% summarise(order_total = sum(total_price), quantity = sum(quantity)) %>% ungroup() %>% summary()
```
