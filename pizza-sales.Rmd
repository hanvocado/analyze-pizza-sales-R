...
+ Phần dƯới đây, sẽ phân tích chi tiết hơn về doanh thu bán được của cửa hàng theo từng phân loại, bao gồm: kích cỡ bánh, danh mục bánh, doanh thu theo từng loại bánh pizza khác nhau.

# 8.1.1. Phân tích doanh thu theo kích cỡ bánh:
...
```
+ Trực quan hóa bảng số liệu bằng biểu đồ cột để so sánh chênh lệch doanh thu giữa các khích cỡ pizza khác nhau:

```{r}
library(scales)
ggplot(revenue_by_size, aes(x = pizza_size, y = revenue, fill = pizza_size)) +
  geom_bar(stat = "identity", color = "black") +
  theme_minimal() +
  labs(
    title = "Doanh thu theo kích cỡ pizza",
    x = "Kích cỡ pizza",
    y = "Doanh thu (USD)",
    fill = "Kích cỡ pizza trong năm 2015"
  ) +
...
# Phân tích thêm về số lượng trong mỗi đơn hàng. 
```
> Qua biểu đồ so sánh, ta thấy rằng pizza kích cỡ L có doanh thu cao nhất, đạt hơn 300.000 USD. Xếp sau đó là kích cỡ M với doanh thu hơn 200.000 USD, tiếp theo là kích cỡ S, M, XL và cuối cùng là kích cỡ XXL. Mặc dù pizza kích cỡ L có giá cao hơn so với hai kích cỡ M và S, nhưng lại bán được nhiều nhất và thu về doanh thu cao nhất trong năm. Điều này cho thấy giá cả của pizza kích cỡ L là hợp lý và được khách hàng ưu tiên chọn lựa.
> Cửa hàng nên tiếp tục duy trì mức giá hợp lý cho pizza kích cỡ L để tối đa hóa doanh thu và lợi nhuận và đưa ra thêm chiến lược khuyến mãi, quảng cáo để thúc đẩy doanh thu cho các kích cỡ có doanh thu thấp hơn như XL và XXL.
