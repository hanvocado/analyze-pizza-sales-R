...
## 6.4. Phân tích và dự đoáns doanh số (sales):
...
sales_data %>% 
  ...
  geom_point(size = 2)
```
Biểu đồ cho thấy có sự biến động lớn trong sales nên khi tạo mô hình hồi quy tuyến tính, em sẽ sử dụng logarithmic transformation trên biến sales.

```{r}
price_sales_model <- lm(log(sales) ~ unit_price * pizza_size + pizza_category, data = sales_data)
...
b) Kiểm định kích cỡ có ảnh hưởng đến doanh số không bằng mô hình hồi quy tuyến tính

```{r}
ggplot(sales_data, aes(x = unit_price, y = log(sales))) +
...
```
Biểu đồ cho thấy có mối quan hệ tương quan giữa doanh số với giá bán, kích cỡ và danh mục.
