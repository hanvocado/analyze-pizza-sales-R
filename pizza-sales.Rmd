...
### 6.4.2. Doanh số theo danh mục bánh
...
#### Kiểm định giá bánh có ảnh hưởng đến doanh số không bằng mô hình hồi quy tuyến tính
...
```{r}
sales_data <- df %>%  group_by(unit_price, pizza_size, pizza_category) %>% summarise(sales = sum(quantity))
...
Biểu đồ cho thấy có sự biến động lớn trong sales và quan hệ giữa unit_price và sales có vẻ uốn cong nên khi tạo mô hình hồi quy tuyến tính, em sẽ sử dụng logarithmic transformation trên biến sales và polynomial regression trên biến unit_price.

* Tạo mô hình hồi quy tuyến tính

```{r}
price_sales_model <- lm(log(sales) ~ poly(unit_price, 2), data = sales_data)
summary(price_sales_model)
```
Giá trị *p-value* là 0.001141, nhỏ hơn mức ý nghĩa thông thường (0.05) nên ta đủ bằng chứng bác bỏ giả thuyết $H_0$ và kết luận giá bánh có ảnh hưởng tới doanh số của nó.

#### Kiểm định kích cỡ có ảnh hưởng đến doanh số không bằng mô hình ANOVA
...
```{r}
sales_model <- lm(log(sales) ~ poly(unit_price, 2) * pizza_size + pizza_category, data = sales_data)
summary(sales_model)
```


```{r}
ggplot(sales_data, aes(x = unit_price, y = total_sales)) +
  geom_jitter(aes(color = pizza_category, shape = pizza_size), size = 2) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = FALSE, color = "blue") +
  labs(title = "Relationship between Unit Price and Sales by Pizza Size and Category",
       x = "Unit Price",
       y = "Total Sales") +
  theme_minimal() +
...
```{r}
ggplot(sales_data, aes(x = pizza_size, y = total_sales, fill = pizza_category)) +
  geom_boxplot() +
  labs(title = "Doanh số theo kích cỡ và danh mục bánh pizza",
       fill = "Danh mục",
       x = "Pizza Size",
       y = "Doanh số") +
  theme_minimal() +
